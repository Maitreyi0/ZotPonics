...
class AtlasI2C:
    ...
    # Returns a list (of objects of class AtlasI2C) of EZO I2C devices currently a part of the I2C network
    # See the methods of this class in AtlasI2C.py
    def get_devices():
        atlasI2C_DeviceList = [] # this is what we want to fill and return
        
        i2cNetwork = AtlasI2C()
        device_address_list = i2cNetwork.list_i2c_devices()
        
        # Filtering the device address list for only the atlas devices
        for i in device_address_list:
            i2cNetwork.set_i2c_address(i)
            response = i2cNetwork.query("I")
            try:
                moduletype = response.split(",")[1] 
                response = i2cNetwork.query("name,?").split(",")[1]
            except IndexError:
                print(">> WARNING: device at I2C address " + str(i) + " has not been identified as an EZO device, and will not be queried") 
                continue
            atlasI2C_DeviceList.append(AtlasI2C(address = i, moduletype = moduletype, name = response))

        return atlasI2C_DeviceList
    
if(__name__ == "__main__"):
    atlasI2C_List = AtlasI2C.get_devices()
    print(atlasI2C_List)
    exit()